...
import { Language, Meeting, Feedback, Lesson, LibraryItem, Question } from './types';
...
const Card: React.FC<{ children: React.ReactNode; className?: string; onClick?: () => void }> = ({ children, className = '', onClick }) => (
  <div 
    onClick={onClick}
    className={`bg-white rounded-2xl shadow-sm border border-slate-100 p-6 ${className} ${onClick ? 'cursor-pointer hover:shadow-md transition-all active:scale-[0.98]' : ''}`}
  >
    {children}
  </div>
);
...
const Button: React.FC<{ 
  ...
  return (
    <button 
      ...
      className={`px-4 py-2 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${variants[variant]} ${className} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100] flex flex-col items-end">
      {isOpen && (
        <div className="w-[380px] h-[500px] bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col mb-4 animate-in slide-in-from-bottom-4 duration-300">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
            {messages.length === 0 && (
              <div className="text-center py-10 text-slate-400">
                ...
                <p className="text-sm font-bold">{lang === 'ar' ? 'اسألني أي شيء عن الحوكمة والتربية المدنية' : 'Ask me anything about governance and civic education'}</p>
              </div>
            )}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-start flex-row-reverse' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm font-bold ${m.role === 'user' ? 'bg-indigo-100 text-indigo-900 rounded-br-none' : 'bg-slate-100 text-slate-800 rounded-bl-none'}`}>
                  {m.text}
                </div>
              </div>
            ))}
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-slate-100 p-3 rounded-2xl rounded-bl-none animate-pulse text-xs text-slate-400 font-bold">... AI is thinking</div>
              </div>
            )}
          </div>
          <div className="p-4 border-t flex gap-2">
            ...
            />
            <Button variant="primary" onClick={sendMessage} className="p-2 h-10 w-10"><Send size={16} /></Button>
          </div>
        </div>
      )}
      <button 
      ...
      >
        ...
        {!isOpen && <span className="absolute -top-1 -left-1 flex h-4 w-4"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span><span className="relative inline-flex rounded-full h-4 w-4 bg-indigo-500"></span></span>}
      </button>
    </div>
  );
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card onClick={() => onNavigate('lessons')} className="group border-l-4 border-l-indigo-500">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{completedCount}/{LESSONS_DATA.length} {lang === 'ar' ? 'مكتمل' : 'Done'}</span>
          </div>
          <h3 className="text-xl font-bold mb-2 group-hover:text-indigo-600 transition-colors">{t.lessons}</h3>
          <p className="text-sm text-slate-500 leading-relaxed">المنهج الوطني للتربية المدنية: {UNITS_METADATA.length} وحدات تعليمية متكاملة لجميع المستويات.</p>
        </Card>
        ...
        <Card onClick={() => onNavigate('toolkit')} className="group border-l-4 border-l-emerald-500">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">نماذج عملية جاهزة للاستخدام: جداول أعمال، مصفوفات أولويات، وأدوات تخطيط مجتمعي.</p>
        </Card>
        ...
        <Card onClick={() => onNavigate('meetings')} className="group border-l-4 border-l-amber-500">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">وثّق اجتماعاتك وقراراتك المحلية لضمان الشفافية والمساءلة وحفظ الحقوق.</p>
        </Card>
        ...
        <Card onClick={() => onNavigate('feedback')} className="group border-l-4 border-l-rose-500">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">آلية تواصل آمنة وسرية لإيصال أصوات المجتمع واحتياجاتهم للجهات المسؤولة.</p>
        </Card>
        ...
        <Card onClick={() => onNavigate('library')} className="group border-l-4 border-l-violet-500">
          ...
          <p className="text-sm text-slate-500 leading-relaxed">مكتبة مرجعية شاملة تضم أدلة تدريبية، مناهج ToT، ونصوص قانونية هامة.</p>
        </Card>
      ...
      <div className="bg-indigo-600 rounded-[2.5rem] p-10 mt-12 flex flex-col md:flex-row items-center gap-10 text-white shadow-2xl shadow-indigo-200">
        <div className="flex-1 text-right">
          ...
          <p className="text-indigo-100 text-lg leading-relaxed mb-8 opacity-90">
            أكاديمية أورانيتيس هي مساحتك الخاصة للتعلم والمبادرة. انضم إلى آلاف السوريين الذين يتعلمون يومياً كيفية بناء مجتمعات أكثر عدلاً وشفافية.
          </p>
          <div className="flex gap-4 justify-end">
            <Button onClick={() => onNavigate('lessons')} className="bg-white !text-indigo-600 hover:bg-indigo-50 px-8 py-4 text-lg">
              {t.startLearning}
            </Button>
            <Button variant="ai" className="px-8 py-4 text-lg" onClick={() => {}}>
...
const LessonsView: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const generateAIInsight = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        Focus on how this lesson helps build a better community in 5 years. Respond in 2-3 short sentences in ${lang === 'ar' ? 'Arabic' : 'English'}.`,
      });
  ...
  const generateExplanation = async () => {
    ...
    try {
      ...
      setAiExplanation("Error generating detailed explanation.");
    } finally {
      setIsAiLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-right-4 duration-300">
        <div className="flex items-center justify-between">
          <Button variant="secondary" onClick={() => { setSelectedLesson(null); setCurrentStep(0); setShowQuizResults(false); setAiInsight(null); setAiSummary(null); setAiExplanation(null); }} className="text-slate-600 bg-white border border-slate-200 shadow-sm flex-row-reverse gap-2">
        ...
        <Card className="p-10 border-slate-100 text-right">
          {currentStep === 0 && (
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <ul className="space-y-4">
                  {selectedLesson.learning_outcomes.map((o, i) => (
                    <li key={i} className="flex items-center justify-end gap-3 text-lg text-slate-600 font-bold">
                      {o} <div className="w-2 h-2 bg-indigo-400 rounded-full"></div>
                    </li>
                  ))}
                </ul>
              </div>
                  ...
                  <div className="bg-violet-50 p-6 rounded-3xl border border-violet-100 relative overflow-hidden flex flex-col h-full">
                    <div className="flex items-center justify-between mb-4 flex-row-reverse">
                      ...
                      {!aiInsight && !isAiLoading && (
                        <Button variant="ai" onClick={generateAIInsight} className="py-1.5 text-[9px] px-2.5">
                          {lang === 'ar' ? 'توليد' : 'Run'}
                        </Button>
                      )}
                    </div>
                  ...
                  <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 relative overflow-hidden flex flex-col h-full">
                    <div className="flex items-center justify-between mb-4 flex-row-reverse">
                      ...
                      {!aiSummary && !isAiLoading && (
                        <Button variant="ai" onClick={generateSummary} className="py-1.5 text-[9px] px-2.5 !bg-indigo-600">
                          {lang === 'ar' ? 'تلخيص' : 'Sum'}
                        </Button>
                      )}
                    </div>
                  ...
                  <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 relative overflow-hidden flex flex-col h-full">
                    <div className="flex items-center justify-between mb-4 flex-row-reverse">
                      ...
                      {!aiExplanation && !isAiLoading && (
                        <Button variant="ai" onClick={generateExplanation} className="py-1.5 text-[9px] px-2.5 !bg-emerald-600">
                ...
                {aiExplanation && (
                  <div className="bg-white p-8 rounded-3xl border-2 border-emerald-100 shadow-xl animate-in zoom-in-95 duration-300">
                    <div className="flex justify-between items-center mb-6 flex-row-reverse border-b border-emerald-50 pb-4">
                       ...
                       <button onClick={() => setAiExplanation(null)} className="text-slate-300 hover:text-slate-500"><X size={20} /></button>
                    </div>
                    <div className="text-slate-700 text-lg leading-relaxed font-bold whitespace-pre-line text-right">
                      {aiExplanation}
                    </div>
                  </div>
                )}
              ...
              <Button onClick={() => setCurrentStep(1)} className="w-full py-4 text-xl">ابدأ الدرس الآن</Button>
            </div>
          )}
          ...
          {currentStep >= 1 && currentStep <= selectedLesson.sections.length && (
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center justify-between mb-4 flex-row-reverse">
                <span className="text-xs font-black text-slate-400 uppercase tracking-widest text-right">قسم {currentStep} من {selectedLesson.sections.length}</span>
                <Lightbulb className="text-amber-400 w-8 h-8" />
              </div>
              <p className="text-2xl leading-relaxed text-slate-700 font-bold">
                {selectedLesson.sections[currentStep - 1]}
              </p>
              <div className="flex gap-4">
                <Button onClick={() => setCurrentStep(currentStep + 1)} className="flex-1 py-4 text-lg">التالي</Button>
                <Button variant="ghost" onClick={() => setCurrentStep(currentStep - 1)} className="py-4 text-lg">السابق</Button>
              </div>
            </div>
          )}
          ...
          {currentStep === selectedLesson.sections.length + 1 && (
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="flex gap-4">
                <Button onClick={() => setCurrentStep(currentStep + 1)} className="flex-1 py-4 text-lg">فهمت، لننتقل للاختبار</Button>
                <Button variant="ghost" onClick={() => setCurrentStep(currentStep - 1)} className="py-4 text-lg">مراجعة الأقسام</Button>
              </div>
            </div>
          )}
          ...
          {currentStep === selectedLesson.sections.length + 2 && (
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="space-y-10">
                {selectedLesson.questions.map((q, i) => (
                  <div key={i} className="space-y-4">
                    <p className="text-xl font-bold text-slate-700">س{i+1}. {q.text}</p>
                      ...
                      {q.type === 'mcq' ? (
                        q.options?.map((opt, optIdx) => (
                          <button
                            ...
                            className={`p-4 rounded-2xl border-2 text-right transition-all font-bold ${quizAnswers[i] === optIdx ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-100 hover:border-indigo-100 text-slate-600'}`}
                          >
                            {opt}
                          </button>
                        ))
                        ...
                        ['صح', 'خطأ'].map((opt, optIdx) => (
                          <button
                            ...
                            className={`p-4 rounded-2xl border-2 text-right transition-all font-bold ${quizAnswers[i] === (optIdx === 0) ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-100 hover:border-indigo-100 text-slate-600'}`}
                          >
                            {opt}
                          </button>
                        ))
                    ...
                    {showQuizResults && (
                      <div className={`p-4 rounded-xl text-sm ${((q.type==='mcq' && quizAnswers[i]===q.correct_index) || (q.type==='truefalse' && quizAnswers[i]===q.correct)) ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                        {q.explanation}
                      </div>
                    )}
                  </div>
              ...
              {!showQuizResults ? (
                <Button onClick={() => setShowQuizResults(true)} className="w-full py-4" disabled={Object.keys(quizAnswers).length < selectedLesson.questions.length}>تأكيد الإجابات</Button>
              ) : (
                <div className="space-y-4">
                  <div className="text-center p-6 bg-slate-50 rounded-3xl">
                    <p className="text-slate-500 font-bold">نتيجتك النهائية</p>
                    <h4 className="text-5xl font-black text-indigo-600 mt-2">{calculateQuizScore()} / {selectedLesson.questions.length}</h4>
                  </div>
                  <Button onClick={() => setCurrentStep(currentStep + 1)} className="w-full py-4 text-lg">خاتمة وتأمل</Button>
                </div>
              )}
            </div>
          )}
               ...
               <div className="bg-[#FDFBF2] rounded-3xl p-8 border border-[#F2E8C4]">
                ...
                <div className="space-y-4 mb-6">
                  {selectedLesson.reflection[lang].map((r, i) => (
                    <p key={i} className="text-[#8C6A2E] text-lg italic leading-relaxed opacity-90">"{r}"</p>
                  ))}
                </div>
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <Card className="bg-indigo-600 text-white p-8 rounded-[2.5rem] flex items-center justify-between shadow-xl flex-row-reverse">
           ...
           <div className="w-48 h-3 bg-white/20 rounded-full overflow-hidden">
              <div className="h-full bg-white transition-all duration-1000" style={{ width: `${p.percent}%` }}></div>
           </div>
        </Card>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           {lessons.map(lesson => (
             <Card key={lesson.id} onClick={() => setSelectedLesson(lesson)} className={`hover:border-indigo-200 transition-all ${progress[lesson.id]?.completed ? 'bg-emerald-50/20 border-emerald-100' : ''}`}>
                <div className="flex justify-between items-start mb-6 flex-row-reverse">
                   <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${lesson.level === 'basic' ? 'bg-blue-50 text-blue-600' : lesson.level === 'advanced' ? 'bg-purple-50 text-purple-600' : 'bg-orange-50 text-orange-600'}`}>
                    {lesson.level}
                   </span>
                ...
                <div className="flex-1 text-right">
                  ...
                  <div className="flex items-center justify-end gap-3 mt-4">
                    <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                       <div className="h-full bg-indigo-500 transition-all duration-1000" style={{ width: `${p.percent}%` }}></div>
                    </div>
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{p.completed}/{p.total}</span>
                  </div>
                </div>
              </div>
            </Card>
...
const ToolkitView: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const [budgetItems, setBudgetItems] = useState(DEFAULT_TEMPLATES.budget);
  const [matrixActivities, setMatrixActivities] = useState(DEFAULT_TEMPLATES.matrix);
  const [surveyQuestions, setSurveyQuestions] = useState(DEFAULT_TEMPLATES.survey);
  ...
  const runAgendaAI = async () => {
    ...
    try {
      ...
      setAiResult("AI Generation Error");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const runBudgetAI = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        Provide 3 cost-saving suggestions and 2 essential items that might be missing for transparency and accountability in a Syrian local context. Respond in ${lang === 'ar' ? 'Arabic' : 'English'}.`,
      });
      setAiResult(response.text || "");
    } catch (e) {
      setAiResult("AI Analysis Error");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const runMatrixAI = async () => {
    ...
    try {
      ...
      setAiResult("AI Prioritization Error");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const runSurveyAI = async () => {
    ...
    try {
      ...
      setAiResult("AI Survey Generation Error");
    } finally {
      setAiLoading(false);
    }
  };
        ...
        <Card className="p-8">
          ...
          <div className="mb-8">
             ...
             <div className="flex gap-4 flex-row-reverse">
                ...
                />
                <Button variant="ai" onClick={runAgendaAI} disabled={aiLoading || !agendaObjective}>
      ...
      <div className="space-y-6 animate-in fade-in duration-300 text-right">
        ...
        <Card className="p-8">
          <div className="flex justify-between items-center mb-6 flex-row-reverse">
            ...
            <Button variant="ai" onClick={runBudgetAI} disabled={aiLoading}>
          ...
          <div className="mt-6 flex justify-end">
            <div className="bg-indigo-600 text-white p-6 rounded-2xl text-right">
              ...
              <h4 className="text-3xl font-black">${budgetItems.reduce((acc, curr) => acc + (curr.qty * curr.cost), 0)}</h4>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="space-y-6 animate-in fade-in duration-300 text-right">
        ...
        <Card className="p-8">
           <div className="flex justify-between items-center mb-8 flex-row-reverse">
              ...
              <Button variant="ai" onClick={runMatrixAI} disabled={aiLoading}>
              ...
              {matrixActivities.map((m, i) => (
                <div key={i} className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                   ...
                   <div className="flex gap-2 flex-row-reverse">
                      <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${m.impact === 'high' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>الأثر: {m.impact}</span>
                      <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${m.effort === 'low' ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>الجهد: {m.effort}</span>
                   </div>
                </div>
              ))}
           </div>
      ...
      <div className="space-y-6 animate-in fade-in duration-300 text-right">
        ...
        <Card className="p-8">
           <div className="flex justify-between items-center mb-8 flex-row-reverse">
              ...
              <Button variant="ai" onClick={runSurveyAI} disabled={aiLoading}>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-right">
      <div className="text-center">
        ...
        <p className="text-slate-500 max-w-2xl mx-auto">مجموعة متكاملة من الأدوات الرقمية لمساعدة اللجان والفرق التطوعية على تنظيم عملها بمهنية وشفافية.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        {tools.map(tool => (
          <Card key={tool.id} onClick={() => setActiveTool(tool.id)} className="flex items-center gap-6 group hover:border-indigo-200 transition-all flex-row-reverse">
...
const MeetingsView: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  const saveMeeting = () => {
    const meeting: Meeting = {
      ...
      decisions: (newMeeting.decisions || []).map(d => ({ ...d, status: d.status || 'pending' }))
    };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-right">
      <div className="flex justify-between items-center flex-row-reverse">
        ...
        <Button onClick={() => setShowForm(true)}><Plus size={20} /> {t.newMeeting}</Button>
      </div>
      ...
      {showForm && (
        <Card className="border-2 border-indigo-100 animate-in slide-in-from-top-4 duration-300">
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              <Button variant="primary" onClick={saveMeeting} className="flex-1 py-4">{t.save}</Button>
              <Button variant="ghost" onClick={() => setShowForm(false)} className="px-8">{t.cancel}</Button>
            </div>
          </div>
        </Card>
      )}
          ...
          meetings.map(m => (
            <Card key={m.id} className="flex flex-row-reverse items-center justify-between group">
              ...
              <Button variant="danger" onClick={() => {
                ...
                setMeetings(updated);
                storage.set(STORAGE_KEYS.MEETINGS, updated);
              }} className="opacity-0 group-hover:opacity-100"><Trash size={18} /></Button>
            </Card>
          ))
        )}
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between flex-row-reverse">
            <div className="flex items-center gap-3 flex-row-reverse">
               ...
               >
                 <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isAnonymous ? 'right-1' : 'right-7'}`}></div>
               </button>
               <span className="text-sm font-bold text-slate-600">{t.anonymous}</span>
            </div>
            <Button variant="primary" onClick={submit} className="bg-rose-600 hover:bg-rose-700 px-8 py-4">{t.submitFeedback}</Button>
          </div>
        </div>
      </Card>
...
--------------------------------------------------
  ...
  const getDocSummary = async (item: LibraryItem) => {
    ...
    try {
      ...
      setAiDocSummary("Error generating AI summary.");
    } finally {
      setIsAiLoading(false);
    }
  };
  ...
  const explainSection = async (index: number, text: string) => {
    ...
    try {
      ...
      setAiSectionExplain(prev => ({ ...prev, [index]: "Error generating deep dive." }));
    } finally {
      setIsAiLoading(false);
    }
  };
  ...
  const askLibrary = async () => {
    ...
    try {
      ...
      setAiSearchResponse("Error querying the Library AI.");
    } finally {
      setIsAiLoading(false);
    }
  };
  ...
  if (selectedItem) {
    return (
      <div className="space-y-6 animate-in fade-in duration-300 text-right">
        <Button variant="secondary" onClick={() => { setSelectedItem(null); setAiDocSummary(null); setAiSectionExplain({}); }} className="flex-row-reverse gap-2">
        ...
        <Card className="p-10">
           <div className="flex justify-between items-start mb-8 flex-row-reverse border-b border-slate-50 pb-6">
              ...
              <Button variant="ai" onClick={() => getDocSummary(selectedItem)} disabled={isAiLoading}>
           ...
           {aiDocSummary && (
             <div className="mb-10 bg-indigo-50/50 p-8 rounded-[2rem] border border-indigo-100 animate-in slide-in-from-top-2">
                ...
                <div className="text-indigo-700 text-lg leading-relaxed font-bold whitespace-pre-line">
                   {aiDocSummary}
                </div>
             </div>
           )}
             ...
             {selectedItem.content?.[lang]?.map((p, i) => (
               <div key={i} className="group relative">
                 <div className="flex items-start gap-6 flex-row-reverse p-6 rounded-[2rem] bg-slate-50/30 border border-transparent hover:border-indigo-100 hover:bg-white transition-all">
                    <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-indigo-600 font-black text-sm flex-shrink-0 shadow-sm">{i+1}</div>
                    <div className="flex-1">
           ...
           <div className="mt-12 pt-8 border-t border-slate-50 flex gap-4 justify-end">
              <Button variant="secondary" onClick={() => handleExportPDF(selectedItem)}><Download size={18} /> تحميل كملف PDF</Button>
              <Button variant="primary" className="bg-indigo-600"><Share2 size={18} /> مشاركة مع الفريق</Button>
           </div>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 text-right">
       <div className="text-center space-y-6 max-w-4xl mx-auto">
         ...
         <p className="text-slate-500 text-xl font-bold leading-relaxed">المستودع الوطني للمعرفة: ابحث في مئات الوثائق القانونية والأدلة التدريبية المعتمدة.</p>
         ...
         <div className="relative group max-w-2xl mx-auto">
            ...
            <button 
              onClick={askLibrary}
              disabled={isAiLoading}
              className="absolute left-6 top-1/2 -translate-y-1/2 bg-slate-100 text-slate-600 px-6 py-2 rounded-2xl font-black text-sm hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
            >
         ...
         {aiSearchResponse && (
           <Card className="bg-gradient-to-br from-indigo-600 to-violet-700 text-white p-10 rounded-[3rem] shadow-2xl text-right animate-in slide-in-from-top-4 relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-6 flex-row-reverse border-b border-white/20 pb-4">
                 ...
                 <button onClick={() => setAiSearchResponse(null)} className="text-white/50 hover:text-white"><X size={20} /></button>
              </div>
              <div className="text-xl leading-relaxed font-bold whitespace-pre-line opacity-95">
                 {aiSearchResponse}
              </div>
           </Card>
         )}
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         {LIBRARY_DATA.map(item => (
           <Card key={item.id} onClick={() => setSelectedItem(item)} className="group border-none shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all h-full flex flex-col p-10 bg-white rounded-[2.5rem]">
              <div className="flex justify-between items-start mb-8 flex-row-reverse">
                 ...
                 <span className="text-[10px] font-black text-slate-400 bg-slate-50 px-3 py-1 rounded-lg uppercase tracking-widest">{item.type}</span>
              </div>
              <h3 className="text-2xl font-black mb-4 text-slate-800 leading-tight group-hover:text-indigo-600 transition-colors">{item.title[lang]}</h3>
      ...
      <Card className="p-8">
        <div className="space-y-8">
          ...
          <div className="border-t border-slate-50 pt-8">
             ...
             }}><Trash size={18} /> مسح ذاكرة التخزين المؤقت</Button>
          </div>
        </div>
      </Card>
    </div>
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'lessons': return <LessonsView lang={lang} />;
      case 'toolkit': return <ToolkitView lang={lang} />;
      case 'meetings': return <MeetingsView lang={lang} />;
      case 'feedback': return <FeedbackView lang={lang} />;
      case 'library': return <LibraryView lang={lang} />;
      case 'settings': return <SettingsView lang={lang} setLang={setLang} />;
  ...
  return (
    <div className={`min-h-screen bg-[#f8fafc] flex font-['Cairo',sans-serif] ${lang === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 z-50 w-80 bg-white border-x border-slate-100 transition-transform duration-300 transform lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : (lang === 'ar' ? 'translate-x-full' : '-translate-x-full')} ${lang === 'ar' ? 'right-0' : 'left-0'}`}>
          ...
          <nav className="space-y-2 flex-1">
            {navigationItems.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all font-bold flex-row-reverse ${view === item.id ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}
              >
                <item.icon size={22} />
                <span className="flex-1 text-right">{item.label}</span>
                {view === item.id && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
              </button>
            ))}
          </nav>
        ...
        <footer className="mt-auto border-t border-slate-100 py-10 px-8">
           <div className="flex flex-col md:flex-row justify-between items-center gap-6 flex-row-reverse">
              ...
              <div className="flex gap-8 text-xs font-black text-slate-400 uppercase tracking-widest">
                 ...
                 <a href="#" className="hover:text-indigo-600 transition-colors">الأمان الرقمي</a>
              </div>
           </div>
        </footer>
      </main>
